...
func (r *Rollback) performRollback(currentRelease, targetRelease *release.Release) (*release.Release, error) {
	...
	// pre-rollback hooks
	if !r.DisableHooks {
		if err := r.cfg.execHook(targetRelease, release.HookPreRollback, r.Timeout, r.DisableVerify); err != nil {
	...
	// post-rollback hooks
	if !r.DisableHooks {
		if err := r.cfg.execHook(targetRelease, release.HookPostRollback, r.Timeout, r.DisableVerify); err != nil {
			return targetRelease, err
		}
	}
